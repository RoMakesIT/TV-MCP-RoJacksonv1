...
export function registerDataTools(server) {
  ...
  server.tool('strategy_get_stats',
    'Extract strategy tester performance stats as a normalized object with known keys: net_pl, profit_factor, win_rate, total_trades, max_drawdown, avg_win, avg_loss, expectancy. Strategy tester panel must be open.',
    { open_panel: z.boolean().optional().describe('Open strategy tester panel first if not visible (default true)') },
    async ({ open_panel = true }) => {
      try {
        if (open_panel) await openPanel({ panel: 'strategy-tester', action: 'open' });
        await new Promise(r => setTimeout(r, 800));
        return jsonResult(await core.getStrategyStats());
      } catch (err) { return jsonResult({ success: false, error: err.message }, true); }
    }
  );
}
