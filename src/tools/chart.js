...
export function registerChartTools(server) {
  ...
  server.tool('chart_get_regime',
    'Compute trend regime (up/down/flat) from EMA crossover and ATR volatility directly from chart bars. No indicators need to be on chart. Returns trend, fast_ema, slow_ema, ema_gap_pct, atr, atr_pct.',
    {
      fast_len: z.number().optional().describe('Fast EMA period (default 50)'),
      slow_len: z.number().optional().describe('Slow EMA period (default 100)'),
      atr_len: z.number().optional().describe('ATR period (default 14)'),
    },
    async ({ fast_len, slow_len, atr_len }) => {
      try { return jsonResult(await core.getRegime({ fast_len, slow_len, atr_len })); }
      catch (err) { return jsonResult({ success: false, error: err.message }, true); }
    }
  );
}
