...
export function registerPineTools(server) {
  ...
  server.tool('pine_run_sweep',
    'Run a multi-pass Pine Script backtest sweep. For each patch: injects source, compiles, waits for strategy tester to recalculate, captures a screenshot, and reads normalized stats. Returns a structured results array for all passes.',
    {
      source: z.string().describe('Base Pine Script source code'),
      patches: z.array(z.object({
        find: z.string().describe('Exact string to find in source'),
        replace: z.string().describe('String to replace it with'),
        label: z.string().describe('Label for this pass (e.g. "off", "block_counter", "smart")'),
      })).describe('One entry per sweep pass'),
      wait_ms: z.number().optional().describe('ms to wait after compile for strategy tester (default 8000)'),
      capture: z.boolean().optional().describe('Screenshot strategy tester after each pass (default true)'),
    },
    async ({ source, patches, wait_ms = 8000, capture = true }) => {
      try { return jsonResult(await core.runSweep({ source, patches, wait_ms, capture })); }
      catch (err) { return jsonResult({ success: false, error: err.message }, true); }
    }
  );
}
