...
export async function getRegime({ fast_len = 50, slow_len = 100, atr_len = 14 } = {}) {
  const lookback = slow_len + 10;
  const raw = await evaluate(`
    (function() {
      var bars = window.TradingViewApi._activeChartWidgetWV.value()._chartWidget.model().mainSeries().bars();
      var last = bars.lastIndex();
      var first = Math.max(bars.firstIndex(), last - ${lookback});
      var result = [];
      for (var i = first; i <= last; i++) {
        var v = bars.valueAt(i);
        if (v) result.push({ o: v[1], h: v[2], l: v[3], c: v[4] });
      }
      return result;
    })()
  `);

  if (!raw?.length) return { success: false, error: 'No bar data available' };
  ...
  const atrBars = raw.slice(-(atr_len + 1));
  let atrSum = 0;
  ...
  const atr = atrSum / atr_len;

  const fastEma = ema(raw, fast_len);
  const slowEma = ema(raw, slow_len);
  const lastClose = raw[raw.length - 1].c;
  const trend = fastEma > slowEma ? 'up' : fastEma < slowEma ? 'down' : 'flat';

  return {
    success: true,
    trend,
    fast_ema: +fastEma.toFixed(4),
    slow_ema: +slowEma.toFixed(4),
    ema_gap_pct: +(Math.abs(fastEma - slowEma) / slowEma * 100).toFixed(3),
    atr: +atr.toFixed(4),
    atr_pct: +(atr / lastClose * 100).toFixed(3),
    last_close: lastClose,
    bars_used: raw.length,
  };
}
