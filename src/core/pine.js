...
import { getStrategyStats } from './data.js';
...
export async function runSweep({ source, patches, wait_ms = 8000, capture = true }) {
  const results = [];

  for (const patch of patches) {
    const patched = source.replace(patch.find, patch.replace);

    await setSource({ source: patched });

    const compRes = await smartCompile();
    const hardErrors = (compRes.errors || []).filter(e => e.severity >= 8);
    if (hardErrors.length > 0) {
      results.push({ label: patch.label, success: false, compile_errors: hardErrors });
      continue;
    }
    ...
    if (capture) {
      try {
        const ss = await captureScreenshot({ region: 'strategy_tester', filename: `sweep_${patch.label}_${Date.now()}` });
        screenshot = ss.file_path || ss.path;
      } catch {}
    }

    const statsRes = await getStrategyStats();
    results.push({ label: patch.label, success: true, stats: statsRes.stats, screenshot });
  }

  return { success: true, passes: results.length, results };
}
