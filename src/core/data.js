...
export async function getStrategyStats() {
  const raw = await evaluate(`
    (function() {
      ...
      if (!panel) return { dom: null };
      var rows = panel.querySelectorAll('[class*="row"], tr');
      var pairs = [];
      rows.forEach(function(row) {
        var spans = Array.from(row.querySelectorAll('span, td, div'))
          .filter(function(el) { return el.children.length === 0 && el.textContent.trim(); });
        if (spans.length >= 2) {
          pairs.push({ label: spans[0].textContent.trim(), value: spans[1].textContent.trim() });
        }
      });
      return { dom: pairs };
    })()
  ...
  const stats = {};
  for (const { label, value } of (raw?.dom || [])) {
    const key = LABEL_MAP[label.toLowerCase()];
    if (key && !(key in stats)) stats[key] = value;
  }

  return { success: Object.keys(stats).length > 0, stats, raw_pairs: raw?.dom?.length ?? 0 };
}
